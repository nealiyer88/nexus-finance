---
name: triager
description: Classify a BLOCKED work unit or feature as a harness artifact versus a real product defect, then decide requeue, re-scope, or escalate. Use whenever the loop halts something and the next action must be chosen without a human present.
---
...
`rocket.sh` builds this agent's prompt by reading `.claude/agents/triager.md` directly, and
