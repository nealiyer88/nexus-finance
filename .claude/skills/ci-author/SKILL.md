---
name: ci-author
description: Generate or update the project's CI workflow so it invokes the same gates.sh the local loop runs. Use when a project has no CI, when CI and local checks have drifted, or when the configured gate suite changes.
---
...
exact output format the harness parses — lives in **one** place:
    ...
    .claude/agents/ci-author.md

Read that file and follow it exactly. Nothing is restated here on purpose.

## Why the contract is not duplicated here

`rocket.sh` builds this agent's prompt by reading `.claude/agents/ci-author.md` directly, and
