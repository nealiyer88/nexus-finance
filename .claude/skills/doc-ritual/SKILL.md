---
name: doc-ritual
description: Update the run log, build record, and learnings files after a feature ships or blocks. Use as the closing step of a feature so the next run starts from an accurate record instead of a stale one.
---
...
exact output format the harness parses — lives in **one** place:
    ...
    .claude/agents/doc-ritual.md

Read that file and follow it exactly. Nothing is restated here on purpose.

## Why the contract is not duplicated here

`rocket.sh` builds this agent's prompt by reading `.claude/agents/doc-ritual.md` directly, and
