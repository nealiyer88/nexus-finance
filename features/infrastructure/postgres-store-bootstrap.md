...
- **Tenant provisioning — owned here, relied on downstream.** `tenants` is declared in both `db/schema.sql` and `db/migrations/001_canonical_schema.sql`; read its DDL by grep at build time rather than trusting a transcription. **Do not assert a count of referencing columns** — earlier queued features keep adding them. Derive the set instead, at build time: every column in `db/schema.sql` whose definition **references the `tenants` table's primary key** — i.e. carries a `REFERENCES` clause naming `tenants` and that key. Match on that reference, not on a fixed spelling of the whole column definition: the DDL aligns types with runs of whitespace, so any pattern that assumes single spaces between the column name, its type, and its constraints finds nothing. Nothing anywhere creates a `tenants` row today, so the first real insert on any of those tables FK-fails. This feature closes that:
...
- **Writers.**
  ...
  - `core/graph/approvals.py` — `record_approval_decision(conn, ...)` writing `approval_decisions`, whose `disposition` column carries a `CHECK` constraint.

    **THE TERMINAL VOCABULARY — DECIDED, NOT TO BE RE-LITIGATED.** The terminal disposition vocabulary is **the value list of the `status` `CHECK` constraint that feature 10b ships in its pending-decisions SQLite migration, minus that constraint's single non-terminal "still pending" value**. This is the same vocabulary the brief already notes 10b "deliberately reuses" for `approval_decisions`, so keying on it makes the brief self-consistent and needs no translation layer. Two earlier revisions tried to source this vocabulary elsewhere; do not reopen the question.

    - **Locate the migration by listing `db/migrations/` at build time** and selecting the SQLite-dialect pending-decisions migration. **This brief writes neither its filename nor its numeric prefix**, by the same rule that governs every other migration reference here.
    - **Parse the `CHECK`'s value list out of that file at build time** and drop the non-terminal value; the remainder is `TERMINAL_SET`. That parse, not this brief, is the authority on the exact strings — **no value is transcribed into this brief, into code, or into a test as a literal.**
    - **Do not key the mapping on `core.matching.types.Action`.** `Action` is a `typing.Literal` of Stage 4 **routing** bands, at least two of which explicitly denote *not yet decided*; it is the wrong vocabulary, it is not an `Enum` so it has no member iteration and no `terminal` attribute.
    - Nothing here may edit 10b's migration, and the derivation must introduce no forbidden token into any file feature 10's grep-guard covers.

    The mapping is an explicit, tested dict from a terminal value to a member of the DDL's `CHECK` set, and may need to normalize case. Read the allowed set out of `approval_decisions.disposition`'s `CHECK` constraint in `db/schema.sql` **by grep at test time**, never by line number and never as a literal list written into a test. The invariant is a **subset assertion in both directions, never a set equality**: `set(MAPPING.values()) <= CHECK_SET_PARSED_FROM_DDL` and `set(MAPPING.keys()) <= TERMINAL_SET`. Both right-hand sides are derived at run time. **There is no producer-set equality assertion anywhere in this feature** — an equality against a set derived by grepping shipped code is exactly the construct that made this criterion vacuous twice.

    **A CHECK value may have no in-tree producer, and that is expected.** Where a value in either derived set is emitted by nothing in `core/`, `api/`, or `dashboard/` — verify by grep at build time rather than naming it here — it stays in the CHECK set and is **unmapped by design**: the mapping simply has no key for it, the brief does not pretend a producer exists, and no producer may be invented. Because both invariants are subset assertions, an unmapped value fails nothing.
...
- [ ] **Scoped to this feature's own diff, not to the whole repo.** Across the lines **this feature adds** under `features/` and `.rocket/` — derived from this feature's commit diff, added lines only — no added line assigns a value to `DATABASE_URL`. The criterion deliberately does **not** grep the repo: other feature briefs, and the gate section of this brief itself, already contain illustrative `DATABASE_URL=` text on a clean checkout, so a repo-wide grep fails on files this feature must not edit and measures pre-existing state rather than what this feature introduces. The substance is unchanged and unconditional: the connection string is **never logged, never placed in an error message, and never written to any file under `features/` or `.rocket/`**; the only committed assignment carrying a value anywhere is the obviously-fake placeholder in `.env.example`.
...
- [ ] `record_approval_decision` inserts an `approval_decisions` row whose `disposition` satisfies the table's CHECK, for every key in the mapping. A no-database unit test parses the CHECK set out of `db/schema.sql` by grep, and parses `TERMINAL_SET` out of the `status` CHECK in 10b's pending-decisions SQLite migration (located by listing `db/migrations/`, never by filename) minus the non-terminal value, then asserts `set(MAPPING.values()) <= CHECK_SET` **and** `set(MAPPING.keys()) <= TERMINAL_SET`. Both are subset assertions; no set equality is asserted and no set may appear as a literal in the test. (mapping: no database; insert: integration)
...
16. **Do not key approval dispositions on `core.matching.types.Action`.** Key on the terminal vocabulary decided above: the `status` CHECK in 10b's pending-decisions SQLite migration (found by listing `db/migrations/`) minus its non-terminal value, parsed at build time; normalize case into the CHECK vocabulary. Both mapping invariants are subset assertions — never a set equality against a set grepped out of shipped code.
