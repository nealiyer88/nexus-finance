...
- **Schema parity:** add `"transactions"` to the hardcoded `SHARED_TABLES` list in `tests/test_schema_parity.py` — that list is the enforcement mechanism, and it compares column names across the two base schema files only.
...
- [ ] `transactions` exists in BOTH `db/schema.sql` and `db/schema_sqlite.sql` with the pinned columns, `UNIQUE (tenant_id, source, external_source_id)`, and the two indexes; `"transactions"` is added to `SHARED_TABLES` in `tests/test_schema_parity.py` and that test passes.
...
- [ ] **(11) Dark by default** — with an empty `transactions` table the helper returns 0, no B3 `BoostEntry` is emitted, and the composite score is byte-identical to the pre-8b value.
